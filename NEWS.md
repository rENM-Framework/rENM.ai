...
* Every report's opening page now carries a REPRODUCIBILITY section, placed
  ...
  realization of a stochastic pipeline, names the seed, says a fixed seed
  makes a run repeatable without making any figure more certain, and
  distinguishes range-wide figures, which reproduce closely, from per-state
  figures for states holding few raster cells, which do not. The manual
  said this in Appendix E, but nobody holding a PDF report has the manual.
* The section is generated by one internal helper and reproduced verbatim
  on both paths, never composed by a model. It makes claims about the
  software -- how the seed is applied, what determinism depends on, which
  figures are stable -- that a model cannot check against anything in its
  context, so a paraphrase would be a false statement about the pipeline
  rather than a rewording. `assemble_coversheet()` renders it directly;
  `submit_to_chatgpt()` and `submit_to_claude()` substitute it into the
  prompt as a `<reproducibility>` block to copy; `render_ai_docx()` verifies
  it came back intact and stops if it did not, on the same reasoning as the
  placeholder check. Whitespace is normalized before that comparison,
  because Word reflows runs within a paragraph; nothing else is.
  The seed is recovered from the `Seed used` lines in the run log and used
  only when every line agrees, so a log holding runs at different seeds
  yields no number and the sentence names a fixed seed without giving a
  value rather than asserting one that was not verified.
  When the multi-run tier lands, the helper returns the aggregate wording
  and both paths follow without either being edited.
* The coversheet matched the AI narrative on colors but not on fonts or page
  setup, and the two are meant to be interchangeable front matter for the
  same report. Three differences, all now resolved by matching the narrative:
  the fonts were inverted, Cambria headings over Calibri body against the
  narrative's Calibri headings over Cambria body -- python-docx's default
  template sets majorFont to Calibri and minorFont to Cambria, which is the
  opposite of what this code assumed; the page was A4, officer's default,
  against the narrative's US Letter; and the margins were 1 inch all round
  against the narrative's 1.25 inch sides. The page size mismatch was
  invisible in assembled reports because `assemble_final_report()` normalizes
  with cpdf `-scale-to-fit`, but that scaled the coversheet down about six
  percent, so its text rendered a size smaller than the same text on the
  narrative's pages. Found by comparing rendered output rather than code.
* INCLUDED FIGURES is now a tight bulleted list rather than eight spaced
  body paragraphs, on the coversheet and in both prompts. On the provider
  path that list shares the narrative's final page with the AI disclosure,
  the new reproducibility block and the citation, and at body spacing the
  eight gaps alone cost about five lines -- enough to push the closing
  timestamp onto a page of its own, which is the orphan the prompt's page
  break section already warns about. Measured on a rendered report rather
  than estimated: the last page is fixed boilerplate in every run, varying
  only in the model name and the timestamp, so what fits once fits always.
  The reproducibility wording was also trimmed, so the page still closes if
  a model ignores the list styling and falls back to spaced paragraphs.
* `.docx_paragraph_text()` returned raw XML instead of text for any
  paragraph carrying a border. The run element was matched with
  `<w:t[^>]*>`, which also matches `<w:top>`, `<w:tabs>` and `<w:tblPr>`;
  the non-greedy tail then ran from that false opening to the first real
  `</w:t>`, swallowing the paragraph's whole property block as if it were
  text. Every officer-written coversheet has such a paragraph, its ruled
  title, so on that path the truncation check in `.check_docx_paragraphs()`
  was inspecting XML, and no XML fragment ends in terminal punctuation.
  Found while verifying the new section against a generated coversheet.
  The narrative path was unaffected, which is why it went unnoticed.
