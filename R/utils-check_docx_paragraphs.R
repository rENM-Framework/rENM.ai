...
#' Check that every fixed block survived the round trip
#'
#' @details
#' The opening page carries text the model is given to copy, not to write:
#' the section headings, the eight figure entries, the reproducibility block
#' and the framework citation. This verifies each one appears as a paragraph
#' equal to what was supplied.
#'
#' The comparison is equality, not containment, which is the point. An
#' earlier version asked whether the document contained the expected string
#' and so accepted anything appended to it. On its second live run a model
...
#' stable between runs, none of which the model can check against anything
#' in its context, so an edit there is a false statement about the pipeline
#' rather than a rewording.
#'
#' Both paths are checked by the same function. The coversheet renders its
#' list with a literal bullet while the narrative uses a list style, where
#' the glyph is not part of the paragraph text, so a leading bullet is
#' stripped before comparison. \code{"AI-ASSISTED INTERPRETATION"} exists
#' only on the provider path and is required only when the document already
...
.check_docx_fixed_text <- function(docx_path, seed = NA_integer_) {
  ...
  repro <- .reproducibility_statement(seed)
  want  <- c(
    .report_headings()[[1L]],
    .report_figures(),
    repro$heading,
  ...
  for (w in norm(want)) {
    if (w %in% have) next
    stem <- substr(w, 1L, min(nchar(w), 28L))
    near <- have[startsWith(have, stem)]
    out <- c(out, if (length(near)) {
      sprintf("altered: expected \"%s\"\n           found \"%s\"",
              w, near[[1L]])
    } else {
